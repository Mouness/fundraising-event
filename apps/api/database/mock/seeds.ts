    ...
    for (const evt of scenarioData.events) {
        ...
        const generateRandomDonation = (index: number) => {
            ...
            const amountTiers = [
                ...
                100000, 250000, 1000000   // High rollers
            ];
            ...
            if (['CASH', 'CHECK', 'PLEDGE'].includes(paymentMethod) && createdStaffMembers.length > 0) {
                staffMemberId = createdStaffMembers[Math.floor(Math.random() * createdStaffMembers.length)].id;
            }
            ...
            return {
                amount,
                currency,
                donorName: isAnonymous ? null : `${donorFirstNames[Math.floor(Math.random() * donorFirstNames.length)]} ${donorLastNames[Math.floor(Math.random() * donorLastNames.length)]}`,
                donorEmail: isAnonymous ? null : `donor${Math.floor(Math.random() * 5000)}@example.com`,
                isAnonymous,
                message,
        ...
        const donations = Array.from({ length: donationCount }).map((_, i) => generateRandomDonation(i));
