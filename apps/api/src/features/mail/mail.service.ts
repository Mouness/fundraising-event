...
export class MailService {
  ...
  async sendReceipt(to: string, data: ReceiptData) {
    ...
    const subject = commConfig.email?.subjectLine || `Receipt for your donation to ${config.content.title}`;
   ...
   */
  private getMailProvider(emailConfig: any): { provider: MailProvider, config: any } {
  ...
  private buildReceiptContext(config: EventConfig, data: ReceiptData): ReceiptContext {
    ...
    return {
      ...
      date: formattedDate,
      content: {
        title: I18nUtil.t(localeData, 'thankyou.receipt.title') === 'thankyou.receipt.title' ? 'Official Donation Receipt' : I18nUtil.t(localeData, 'thankyou.receipt.title'), // Fallback if key missing
        ...
        thankYou: I18nUtil.t(localeData, 'thankyou.receipt.message_body', { donorName: data.donorName || data.name || 'Supporter' }), // Need to add this key or fallback
        authorizedSignature: I18nUtil.t(localeData, 'thankyou.receipt.authorized_signature'), // Need key
  ...
  private async generatePdfAttachment(context: ReceiptContext): Promise<{ filename: string; content: Buffer }[]> {
    try {
