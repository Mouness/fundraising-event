...
export class ConsoleMailProvider implements MailProvider {
    ...
    async send(to: string, subject: string, template: string, context: any, attachments?: { filename: string, content: Buffer }[]): Promise<void> {
        ...
        if (attachments?.length) {
            ...
            attachments.forEach(att => this.logger.log(` - ${att.filename} (${att.content.length} bytes)`));
        }
