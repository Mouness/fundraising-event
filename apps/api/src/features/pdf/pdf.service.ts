...
export class PdfService {
    ...
    async generateReceipt(arg1: string | ReceiptContext, arg2?: ReceiptData): Promise<Buffer> {
        const context = typeof arg1 === 'string' ? await this.buildContext(arg1, arg2!) : arg1
    ...
    private async renderReceipt(context: ReceiptContext): Promise<Buffer> {
        ...
        try {
            ...
            if (context.signatureImage)
                signatureImage = await this.fetchImage(context.signatureImage as string)
        } catch (error) {
            this.logger.warn(`Failed to load images for PDF: ${error.message}`)
        }

        const docDefinition = getReceiptTemplate(context, {
            logo: logoImage,
            ...
            content: {
                title:
                    I18nUtil.t(localeData, 'thankyou.receipt.title') === 'thankyou.receipt.title'
                        ...
                        : I18nUtil.t(localeData, 'thankyou.receipt.title'),
