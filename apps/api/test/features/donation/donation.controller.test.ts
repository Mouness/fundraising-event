...
describe('DonationController', () => {
    ...
    describe('createPaymentIntent', () => {
        it('should delegate to payment service', async () => {
            mockPaymentService.createPaymentIntent.mockResolvedValue({ clientSecret: 'abc' });
            const result = await controller.createPaymentIntent({ amount: 1000 });
            expect(result).toEqual({ clientSecret: 'abc' });
            expect(mockPaymentService.createPaymentIntent).toHaveBeenCalledWith(1000, 'usd', undefined);
        });

        it('should throw BadRequest if amount is invalid', async () => {
            await expect(controller.createPaymentIntent({ amount: 0 }))
