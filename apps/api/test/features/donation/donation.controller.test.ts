...
describe('DonationController', () => {
    ...
    describe('createPaymentIntent', () => {
        ...
        it('should throw BadRequest if amount is invalid', async () => {
            await expect(controller.createPaymentIntent({ amount: 0 })).rejects.toThrow(
                BadRequestException,
            )
        })
    })
    ...
    describe('handleStripeWebhook', () => {
        ...
        it('should throw BadRequest if stripe event construction fails', async () => {
            mockPaymentService.constructEventFromPayload.mockRejectedValue(
                new Error('Stripe Error'),
            )
            const req = { rawBody: Buffer.from('raw'), headers: {} } as any
            await expect(controller.handleStripeWebhook(req)).rejects.toThrow(
                'Webhook Error: Stripe Error',
            )
        })
    })
    ...
    describe('handlePayPalWebhook', () => {
        ...
        it('should throw BadRequest if paypal event construction fails', async () => {
            ...
            const req = { rawBody: Buffer.from('raw'), headers: {} } as any
            await expect(controller.handlePayPalWebhook(req)).rejects.toThrow(
                'PayPal Webhook Error: PayPal Error',
            )
        })
    })
    ...
    describe('createOfflineDonation', () => {
        ...
        it('should throw BadRequest if eventId is missing', async () => {
            ...
            await expect(controller.createOfflineDonation(dto, req)).rejects.toThrow(
