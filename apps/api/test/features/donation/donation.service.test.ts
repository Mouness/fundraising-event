...
describe('DonationService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw error for invalid provided eventId', async () => {
      ...
      await expect(service.create(data)).rejects.toThrow('No event found to link donation to.');
    });
  ...
  describe('cancel', () => {
    ...
    it('should throw BadRequestException if donation is already cancelled', async () => {
      mockPrismaService.donation.findUnique.mockResolvedValue({ status: 'CANCELLED' });
      await expect(service.cancel('d1')).rejects.toThrow(BadRequestException);
    });

    it('should cancel donation without refund', async () => {
      mockPrismaService.donation.findUnique.mockResolvedValue({ id: 'd1', status: 'COMPLETED' });
    ...
    it('should throw BadRequestException if missing transactionId for stripe refund', async () => {
      ...
      await expect(service.cancel('d1', true)).rejects.toThrow('Cannot refund: Missing Transaction ID');
    });
  ...
  describe('findAll', () => {
    it('should return paginated donations', async () => {
      ...
      expect(mockPrismaService.donation.findMany).toHaveBeenCalledWith(expect.objectContaining({
