        ...
        <Card style={{ backgroundColor: 'var(--admin-card-bg)', borderColor: 'var(--admin-border-color)' }}>
            ...
            <CardContent>
                <div className="space-y-8">
                    {donations.length === 0 ? (
                        <p className="text-sm text-muted-foreground">{t('dashboard.activity.empty', 'No recent activity')}</p>
                    ) : (
                        donations.map((donation) => (
                            <div key={donation.id} className="flex items-center">
                                <div className="h-9 w-9 rounded-full flex items-center justify-center relative">
                                    <div className="absolute inset-0 rounded-full opacity-10" style={{ backgroundColor: 'var(--primary)' }} />
                                    <DollarSign className="h-4 w-4 relative" style={{ color: 'var(--primary)' }} />
                                </div>
                                <div className="ml-4 space-y-1">
                                    <p className="text-sm font-medium leading-none">
                                        {donation.isAnonymous ? t('common.anonymous', 'Anonymous') : donation.donorName}
                                    </p>
                                ...
                                <div className="ml-auto font-medium" style={{ color: 'var(--global-success-color)' }}>
