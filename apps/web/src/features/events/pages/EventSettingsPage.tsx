...
export const EventSettingsPage = () => {
    ...
    const navItems = [
        {
            id: 'general',
            label: t('event_settings.nav.general', 'General'),
            icon: Settings,
        },
        {
            id: 'branding',
            label: t('event_settings.nav.branding', 'Design & Branding'),
            icon: Palette,
        },
    ]
    ...
    useEffect(() => {
        ...
        form.reset({
            ...
            formConfig: {
                phone: {
                    enabled: donationForm?.phone?.enabled ?? false,
                    required: false,
                },
                address: {
                    enabled: donationForm?.address?.enabled ?? false,
                    required: false,
                },
                company: {
                    enabled: donationForm?.company?.enabled ?? false,
                    required: false,
                },
                message: {
                    enabled: donationForm?.message?.enabled ?? true,
                    required: false,
                },
                anonymous: {
                    enabled: donationForm?.anonymous?.enabled ?? true,
                    required: false,
                },
            },
            // Branding
    ...
    const mutation = useMutation({
        mutationFn: async (values: z.infer<typeof eventSettingsSchema>) => {
            ...
            if (values.useGlobalBranding) {
                ...
                const variablesMap = (values.themeVariables || []).reduce(
                ...
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h2 className="text-2xl font-bold tracking-tight">
                            {t('event_settings.title', 'Event Settings')}
                        </h2>
                        <p className="text-muted-foreground">
                            {t(
                                'event_settings.subtitle',
                                'Configure event details and appearance.',
                            )}
                        </p>
                    </div>
