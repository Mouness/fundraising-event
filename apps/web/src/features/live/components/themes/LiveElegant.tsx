...
export const LiveElegant = ({ config, donations, totalRaisedCents, prevTotal, activeSlug }: LiveThemeProps) => {
    ...
    return (
        ...
        >
            ...
            {bgImage && <div className="absolute inset-0 bg-slate-900/80 z-0 pointer-events-none" />}

            {/* Elegant Background Pattern - Only if no BG Image */}
            {!bgImage && <div className="absolute inset-0 z-0 opacity-20" style={{ backgroundImage: 'radial-gradient(circle at 2px 2px, #94a3b8 1px, transparent 0)', backgroundSize: '40px 40px' }}></div>}

            {/* Gold Glows - Keep these as they add atmosphere, but maybe reduce opacity if BG image is present */}
            <div className={`absolute top-0 left-1/4 w-[600px] h-[600px] bg-amber-500/10 rounded-full blur-[120px] pointer-events-none ${bgImage ? 'opacity-50' : ''}`}></div>
            <div className={`absolute bottom-0 right-1/4 w-[600px] h-[600px] bg-indigo-500/10 rounded-full blur-[120px] pointer-events-none ${bgImage ? 'opacity-50' : ''}`}></div>
                ...
                <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
                    ...
                    <div className="hidden lg:flex lg:col-span-3 flex-col justify-end h-full pb-12">
                        <div
                            ...
                            style={{ backgroundColor: 'var(--live-elegant-card-bg, rgba(30, 41, 59, 0.5))' }}
                        >
                            ...
                            <p className="text-amber-100/80 font-serif italic text-lg">{t('live.scan_to_contribute', 'Scan to contribute')}</p>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-3 h-[500px] flex flex-col relative">
                        ...
                        <h2 className="text-xl font-serif text-amber-50 mb-8 pl-4">{t('live.recent_gifts', 'Recent Gifts')}</h2>
