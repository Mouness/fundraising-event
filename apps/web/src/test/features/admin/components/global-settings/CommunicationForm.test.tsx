...
describe('CommunicationForm', () => {
    it('renders contact points', () => {
        ...
        expect(screen.getByLabelText('admin_branding.communication.contact.email')).toHaveValue(
            'test@example.com',
        )
        expect(screen.getByLabelText('admin_branding.communication.contact.phone')).toHaveValue(
            '123456',
        )
        expect(screen.getByLabelText('admin_branding.communication.contact.address')).toHaveValue(
            'Main St',
        )
    })
    ...
    it('shows sharing settings when enabled', () => {
        ...
        const fbCheckbox = screen.getByRole('checkbox', {
            name: /admin_branding.communication.sharing.facebook/i,
        })
