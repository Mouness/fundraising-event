...
describe('LocalizationForm', () => {
    ...
    it('adds a new locale override', () => {
        ...
        expect(setLocaleOverrides).toHaveBeenCalledWith([{ key: 'new.key', value: '', locale: 'en' }]);
    });
    ...
    it('updates and removes existing overrides', () => {
        ...
        render(
            <Wrapper>
                <LocalizationForm localeOverrides={overrides} setLocaleOverrides={setLocaleOverrides} />
            </Wrapper>
        );

        const input = screen.getByDisplayValue('Hello');
        fireEvent.change(input, { target: { value: 'Bonjour' } });
        expect(setLocaleOverrides).toHaveBeenCalledWith([{ key: 'existing.key', value: 'Bonjour', locale: 'en' }]);
