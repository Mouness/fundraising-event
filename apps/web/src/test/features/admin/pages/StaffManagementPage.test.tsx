...
describe('StaffManagementPage', () => {
    ...
    it('allows adding a staff member', async () => {
        ...
        fireEvent.change(screen.getByLabelText('admin_team.member_name'), { target: { value: 'New Staff' } });
        fireEvent.change(screen.getByLabelText('admin_team.pin_code'), { target: { value: '5555' } });

        const dialog = await screen.findByRole('dialog');
        const submitBtn = within(dialog).getByRole('button', { name: 'admin_team.add_member' }); // Dialog footer button
        await user.click(submitBtn);

        await waitFor(() => expect(api.post).toHaveBeenCalledWith('/staff', { name: 'New Staff', code: '5555' }));
        expect(toast.success).toHaveBeenCalledWith('admin_team.member_created');
    });

    it('allows editing a staff member', async () => {
        const user = userEvent.setup();
        const staffMember = { id: '1', name: 'John Doe', code: '1234', _count: { events: 0 } };
        (api.get as any).mockResolvedValue({ data: [staffMember] });
        (api.put as any).mockResolvedValue({ data: { ...staffMember, name: 'John Updated' } });

        render(<StaffManagementPage />, { wrapper: Wrapper });

        await waitFor(() => expect(screen.getByText('John Doe')).toBeDefined());

        const editBtn = screen.getAllByRole('button').find(b => b.querySelector('.lucide-square-pen')); // lucide-edit is square-pen now
        if (editBtn) await user.click(editBtn);

        await screen.findByRole('dialog');

        const dialog = await screen.findByRole('dialog');
        fireEvent.change(within(dialog).getByLabelText('admin_team.member_name'), { target: { value: 'John Updated' } });
        const saveBtn = within(dialog).getByText('admin_team.save_changes');
        await user.click(saveBtn);

        await waitFor(() => expect(api.put).toHaveBeenCalledWith('/staff/1', { name: 'John Updated', code: '1234' }));
