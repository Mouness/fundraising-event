...
describe('CheckoutForm', () => {
    ...
    it('should submit details and show payment form', async () => {
        ;(api.post as any).mockResolvedValue({
            data: { id: 'sess1', clientSecret: 'cs_123' },
        })

        render(<CheckoutForm />)

        await act(async () => {
            fireEvent.change(screen.getByLabelText(/donation.name/i), {
                target: { value: 'John Doe' },
            })
            fireEvent.change(screen.getByLabelText(/donation.email/i), {
