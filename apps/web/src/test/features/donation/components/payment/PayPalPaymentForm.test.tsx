...
vi.mock('@paypal/react-paypal-js', () => ({
    ...
    PayPalButtons: ({ createOrder, onApprove, onError }: any) => (
        <div>
            <button
                onClick={() =>
                    createOrder({}, { order: { create: vi.fn().mockResolvedValue('tr_999') } })
                }
            >
                Mock Create Order
            </button>
            <button
                onClick={() =>
                    onApprove(
                        {},
                        { order: { capture: vi.fn().mockResolvedValue({ id: 'tr_123' }) } },
                    )
                }
            >
                Mock Approve
            ...
            <button onClick={() => onError(new Error('PayPal Widget Error'))}>
                Mock Widget Error
            </button>
        </div>
    ),
