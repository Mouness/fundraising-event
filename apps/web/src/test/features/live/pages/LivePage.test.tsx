...
vi.mock('../../../../features/live/components/themes/LiveClassic', () => ({
    LiveClassic: (props: any) => <div data-testid="live-classic">Classic Theme: {props.totalRaisedCents}</div>
}));
vi.mock('../../../../features/live/components/themes/LiveModern', () => ({
    LiveModern: (props: any) => <div data-testid="live-modern">Modern Theme: {props.totalRaisedCents}</div>
}));
vi.mock('../../../../features/live/components/themes/LiveElegant', () => ({
    LiveElegant: (props: any) => <div data-testid="live-elegant">Elegant Theme: {props.totalRaisedCents}</div>
}));
...
describe('LivePage', () => {
    ...
    it('renders the correct theme based on config', () => {
        ...
        // Modern
        (useAppConfig as any).mockReturnValue({ config: { ...mockConfig, live: { theme: 'modern' } } });
        render(<LivePage />);
        expect(screen.getByTestId('live-modern')).toBeInTheDocument();

        // Elegant
        (useAppConfig as any).mockReturnValue({ config: { ...mockConfig, live: { theme: 'elegant' } } });
        render(<LivePage />);
    ...
    it('initializes state from fetched event data', async () => {
        vi.mocked(api.get).mockResolvedValue({
            data: {
                ...
                donations: [
                    { id: 'd1', amount: 5000, donorName: 'Alice', createdAt: new Date().toISOString() }
                ]
            }
        });
    ...
    it('updates total updates on socket event', async () => {
        ...
        (useLiveSocket as any).mockReturnValue({
            lastEvent: { amount: 5000, donorName: 'John', isAnonymous: false, currency: 'USD' }
        });
    ...
    it('fires confetti on large donation', () => {
        ...
        (useLiveSocket as any).mockReturnValue({
            lastEvent: { amount: 5000, donorName: 'Big Spender', isAnonymous: false, currency: 'USD' }
        });
    ...
    it('fires goal celebration when goal reached', async () => {
        ...
        // 1. Almost there
        (useLiveSocket as any).mockReturnValue({
            lastEvent: { amount: 9000, donorName: 'A', isAnonymous: false, currency: 'USD' }
        });
        rerender(<LivePage />);

        // 2. Cross the line
        (useLiveSocket as any).mockReturnValue({
            lastEvent: { amount: 2000, donorName: 'B', isAnonymous: false, currency: 'USD' }
        });
        rerender(<LivePage />);
