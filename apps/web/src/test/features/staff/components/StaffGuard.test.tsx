...
describe('StaffGuard', () => {
    ...
    it('should show nothing while loading event', () => {
        vi.mocked(useStaffAuth).mockReturnValue({ isStaffAuthenticated: vi.fn() } as any);
        vi.mocked(useEvent).mockReturnValue({ event: null, isLoading: true } as any);
    ...
    it('should redirect to login if not authenticated', () => {
        vi.mocked(useStaffAuth).mockReturnValue({ isStaffAuthenticated: vi.fn().mockReturnValue(false) } as any);
        vi.mocked(useEvent).mockReturnValue({ event: { id: 'evt-1' }, isLoading: false } as any);
    ...
    it('should render outlet if authenticated', () => {
        vi.mocked(useStaffAuth).mockReturnValue({ isStaffAuthenticated: vi.fn().mockReturnValue(true) } as any);
        vi.mocked(useEvent).mockReturnValue({ event: { id: 'evt-1' }, isLoading: false } as any);
